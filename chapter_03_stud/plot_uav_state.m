function plot_mav_state(uu)
%
% modified 12/11/2009 - RB
% modified 09/06/2024 - PF
    ...
    aircraftViewEnabled = uu(55);    % is figure(1) plotted in parallel?
    ...
    chi = unwrap_angle(chi,chi_prev);

    % first time function is called, initialize plot and persistent vars
    if t==0
        f = figure(2);
        clf;
        f.Position = [0 50 600 800];

        subplot(8,2,1)
        hold on
        pn_handle = graph_y_yhat_yd(t, pn, pn_hat, pn_c, 'p_n, m', []);
        
        subplot(8,2,2)
        hold on
        Va_handle = graph_y_yhat_yd(t, Va, Va_hat, Va_c, 'V_a, m/s', []);

        subplot(8,2,3)
        hold on
        pe_handle = graph_y_yhat_yd(t, pe, pe_hat, pe_c, 'p_e, m', []);

        subplot(8,2,4)
        hold on
        alpha_handle = graph_y_yhat_yd(t, alpha, alpha_hat, alpha_c, '\alpha, deg', []);

        subplot(8,2,5)
        hold on
        h_handle = graph_y_yhat_yd(t, h, h_hat, h_c, 'h, m', []);

        subplot(8,2,6)
        hold on
        beta_handle = graph_y_yhat_yd(t, beta, beta_hat, beta_c, '\beta, deg', []);

        subplot(8,2,7)
        hold on
        phi_handle = graph_y_yhat_yd(t, phi, phi_hat, phi_c, '\phi, deg', []);
        
        subplot(8,2,8)
        hold on
        p_handle = graph_y_yhat_yd(t, p, p_hat, p_c, 'p, deg/s', []);
        
        subplot(8,2,9)
        hold on
        theta_handle = graph_y_yhat_yd(t, theta, theta_hat, theta_c, '\theta, deg', []);
        
        subplot(8,2,10)
        hold on
        q_handle = graph_y_yhat_yd(t, q, q_hat, q_c, 'q, deg/s', []);
        
        subplot(8,2,11)
        hold on
        chi_handle = graph_y_yhat_yd(t, chi, chi_hat, chi_c, '\chi, deg', []);
        
        subplot(8,2,12)
        hold on
        r_handle = graph_y_yhat_yd(t, r, r_hat, r_c, 'r, deg/s', []);
        ...
        ylabel('\delta_t\in[0,1]')  
    % at every other time step, redraw state variables
    else
       if aircraftViewEnabled~=0  % if figure(1) is plotted in parallel
        figure(2);
       end
